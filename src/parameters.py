...
def load_parameters(default_parameters={},
    ...
    if configuration_file:
        with open(configuration_file) as f:
            ...
            output_files = {key: os.path.join(output_directory, path)
                            for key, path in output_filenames.items()}
    else:
        parameters = default_parameters
        output_files = {key: os.path.join(root, default_output_directory, path)
                        for key, path in output_filenames.items()}
